...
public final class BranchTracer implements IBranchTracer {
    ...
    public void passedBranch(final int i, final int opcode, final int trueBranch, final int falseBranch) {
        ...
        switch (opcode) {
            case 153 -> { // IFEQ (i == 0)
                distanceTrue = Math.abs(i);  // True if i == 0 (distance = |i|)
                distanceFalse = (i == 0) ? 1.0 : 0.0;           // Always false if i ≠ 0
            }
            case 154 -> { // IFNE (i != 0)
                distanceTrue = (i != 0) ? 0.0 : 1.0;            // Always true if i ≠ 0
                distanceFalse = Math.abs(i); // False if i == 0 (distance = |i|)
            }
            case 155 -> { // IFLT (i < 0)
                distanceTrue = Math.max(0, i + 1); // True if i < 0 (distance = max(0, i + 1))
                distanceFalse = (i < 0) ? (Math.abs(i)) : 0.0;      // False if i >= 0 (distance = |i|)
            }
            case 156 -> { // IFGE (i >= 0)
                distanceTrue = (i >= 0) ? 0.0 : Math.abs(i);       // True if i >= 0 (distance = |i|)
                distanceFalse = (i >= 0) ? Math.abs(i) + 1 : 0.0; // False if i < 0
            }
            case 157 -> { // IFGT (i > 0)
                distanceTrue = (i > 0) ? 0.0 : Math.abs(i) + 1; // True if i > 0
                distanceFalse = (i > 0) ? i : 0.0;       // False if i <= 0
            }
            case 158 -> { // IFLE (i <= 0)
                distanceTrue = (i <= 0) ? 0.0 : i;       // True if i <= 0
                distanceFalse = (i <= 0) ? Math.abs(i) + 1 : 0.0; // False if i > 0
            }
    ...
    public void passedBranch(final int i, final int j, final int opcode, final int trueBranch,
        ...
        switch (opcode) {
            case 159 -> { // IF_ICMPEQ (i == j)
                distanceTrue = Math.abs(i - j);  // Distance is |i - j| for true condition
                distanceFalse = (i == j) ? 1.0 : 0.0; // Always false if i ≠ j
            }
            case 160 -> { // IF_ICMPNE (i != j)
                distanceTrue = (i != j) ? 0.0 : 1.0;  // Always true if i ≠ j
                distanceFalse = Math.abs(i - j); // Distance is |i - j| for false condition
            }
            case 161 -> { // IF_ICMPLT (i < j)
                distanceTrue = Math.max(0, i - j + 1); // Distance is max(0, i - j + 1)
                distanceFalse = (i < j) ? Math.abs(i - j) : 0.0;  // False branch distance
            }
            case 162 -> { // IF_ICMPGE (i >= j)
                distanceTrue = (i >= j) ? 0.0 : Math.abs(j - i); // True if i >= j
                distanceFalse = (i >= j) ? Math.abs(j - i) + 1 : 0.0; // False if i < j
            }
            case 163 -> { // IF_ICMPGT (i > j)
                distanceTrue = (i > j) ? 0.0 : j - i + 1; // True if i > j
                distanceFalse = (i > j) ? Math.max(0, i - j) : 0.0; // False if i <= j
            }
            case 164 -> { // IF_ICMPLE (i <= j)
                distanceTrue = (i <= j) ? 0.0 : Math.abs(i - j); // True if i <= j
                distanceFalse = (i <= j) ? Math.abs(i - j) + 1 : 0.0; // False if i > j
            }
            default -> throw new IllegalArgumentException("Unknown opcode: " + opcode);
        }
    ...
    public void passedBranch(final Object o, final int opcode, final int trueBranch,
        ...
        switch (opcode) {
            case 198 -> { // IFNULL (o == null)
                distanceTrue = (o == null) ? 0.0 : 1.0;  // If o == null, distance is 0. Otherwise, 1.
                distanceFalse = (o == null) ? 1.0 : 0.0; // If o != null, distance is 0.
            }
            case 199 -> { // IFNONNULL (o != null)
                distanceTrue = (o != null) ? 0.0 : 1.0;  // If o != null, distance is 0.
                distanceFalse = (o != null) ? 1.0 : 0.0; // If o == null, distance is 0.
            }
    ...
    private void traceBranchDistance(final int trueBranch, final double distanceTrue, final int falseBranch, final double distanceFalse) {
        ...
        if (distanceTrue < 0 || distanceFalse < 0) {
            throw new IllegalArgumentException("Branch distances must be non-negative.");
        }

        distances.compute(trueBranch, (key, existingDistance) ->
                (existingDistance == null || distanceTrue < existingDistance) ? distanceTrue : existingDistance
        );

        distances.compute(falseBranch, (key, existingDistance) ->
                (existingDistance == null || distanceFalse < existingDistance) ? distanceFalse : existingDistance
        );
    }
