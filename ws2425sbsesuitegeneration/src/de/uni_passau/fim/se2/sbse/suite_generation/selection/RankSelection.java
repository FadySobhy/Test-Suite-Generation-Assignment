...
public class RankSelection<C extends Chromosome<C>> implements Selection<C> {
    ...
    public C apply(final List<C> population) {
        ...
        if (population.size() != size) {
            throw new IllegalArgumentException("Population size must match initialized size.");
        }
        ...
        for (int i = 0; i < N; i++) {
            probabilities[i] = (2 - bias + 2 * (bias - 1) * i / (N - 1.0)) / N;
        }
        ...
        return sortedPopulation.get(N - 1);
    }
}
