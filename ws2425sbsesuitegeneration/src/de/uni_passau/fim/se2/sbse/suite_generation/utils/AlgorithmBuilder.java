...
public class AlgorithmBuilder {
    ...
    private GeneticAlgorithm<?> buildMOSA() {
        ...
        // Ensure we have branches to cover
        if (branchesToCover.isEmpty()) {
            throw new IllegalStateException("No branches to cover for MOSA.");
        }

        // Create chromosome generator
        ChromosomeGenerator<TestCaseChromosome> chromosomeGenerator =
                new TestCaseChromosomeGenerator(random, testGenerationTarget);

        // Create mutation and crossover operators
        Mutation<TestCaseChromosome> mutation = new TestCaseMutation(random);
        Crossover<TestCaseChromosome> crossover = new TestCaseCrossover(random);

        // Define fitness function using branch coverage
        FitnessFunction<TestCaseChromosome> fitnessFunction =
                new BranchCoverageFitness(branchTracer, branchesToCover);

        // Build and return the MOSA genetic algorithm
        return new MOSA<>(
                chromosomeGenerator,
                populationSize,
                fitnessFunction,
    ...
    private GeneticAlgorithm<?> buildRandomSearch() {
        ...
        // Ensure we have branches to cover
        if (branchesToCover.isEmpty()) {
            throw new IllegalStateException("No branches to cover for Random Search.");
        }

        return new RandomSearch<>(
                TestCaseChromosome::random, // Generates random test cases
                fitnessFunction, // Measures branch coverage
                stoppingCondition, // Defines termination condition
                Randomness.random() // Injects shared randomness instance
        );
    }
}
